...
Not yet. Each visitor message is treated independently — the bot doesn't currently use prior turns as context. Multi-turn context is on the roadmap.

In practice, this means visitors get the best results when each question is self-contained (*"what are your refund policies?"* not *"what about the previous one?"*).
...
Data is retained while your account is open. The widget on your site stops responding (returns an error). Deletion-on-request is available — email `hello@tenloai.com`. See [Pricing & billing](/support-chatbot/reference/billing) for the full retention window.
